...
contract Global {
    ...
    {
        return (
            decentralizedBank.users,
            decentralizedBank.companies,
            decentralizedBank.transactions,
            decentralizedBank.products
        );
    }
    ...
    function newUser() external {
        ...
        bytes32 uid =
            stringToBytes32(
                string(abi.encodePacked("user_", uint2str(user_count)))
            );
        User memory newuser =
            User({
                id: uid,
    ...
    function newCompany() external {
        ...
        bytes32 cid =
            stringToBytes32(
                string(abi.encodePacked("company_", uint2str(company_count)))
            );
