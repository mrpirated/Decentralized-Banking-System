...
contract Functions is Global {
    ...
    function payTaxes() external {
     ...
     function remove(uint256 index, bytes32 id) internal {
        if (index >= lenders[id].clients.length) return;

        lenders[id].clients[index] = lenders[id].clients[
            lenders[id].clients.length - 1
        ];
        lenders[id].amount[index] = lenders[id].amount[
            lenders[id].months.length - 1
        ];
        lenders[id].months[index] = lenders[id].months[
            lenders[id].amount.length - 1
        ];
    ...
    function monthly() external {
        for (uint256 i = 0; i < decentralizedBank.users.length; i++) {
            if (users[decentralizedBank.users[i]].isUser) {
                ...
                if (lenders[id].islender) {
                    for (uint256 j = 0; j < lenders[id].clients.length; j++) {
                        lenders[id].months[j]--;
                        if (lenders[id].months[j] == 0) {
                            if (
                                users[lenders[id].clients[j]].inhand >=
                                lenders[id].amount[j]
                            ) {
                                transaction_count++;
                                bytes32 tid =
                                ...
                                users[lenders[id].clients[j]].inhand -= lenders[
                                    id
                                ]
                                    .amount[j];
                                users[lenders[id].clients[j]].income -= lenders[
                                    id
                                ]
                                    .amount[j];
                                users[id].inhand += lenders[id].amount[j];
                                users[id].income += lenders[id].amount[j];
                                users[lenders[id].clients[j]].transactions.push(
                                    tid
                                );
                            ...
                            users[id].net_worth +=
                                (lenders[id].amount[j] *
                                    (lenders[id].ipm + lenders[id].iipm * -1)) /
                                100;
                            users[lenders[id].clients[j]].net_worth -=
                                (lenders[id].amount[j] *
                                    (lenders[id].ipm + lenders[id].iipm * -1)) /
                                100;
                            lenders[id].amount[j] +=
        ...
        for (uint256 i = 0; i < decentralizedBank.companies.length; i++) {
            if (companies[decentralizedBank.companies[i]].isCompany) {
                int256 amount =
                    companies[decentralizedBank.companies[i]].salary;
                for (
                    uint256 j = 0;
                    j <
                    companies[decentralizedBank.companies[i]].employees.length;
                    j++
                ) {
                    transaction_count++;
                    bytes32 tid =
                    ...
                    Transaction memory newtransaction =
                        Transaction(
                            ...
                            true,
                            decentralizedBank.companies[i],
                            companies[decentralizedBank.companies[i]].employees[
                                j
                            ],
                            amount,
                            false,
    ...
    function productPurchase(
        ...
        if (
            companies[products[productId].company].isCompany &&
            users[fromid].isUser &&
            products[productId].isProduct
        ) {
            transaction_count++;
            bytes32 tid =
                stringToBytes32(
            ...
            Transaction memory newtransaction =
                Transaction(
                    ...
                    true,
                    fromid,
                    products[productId].company,
                    amount,
                    false,
                    ""
                );
            if (
                products[productId].selling_price * quantity <=
                users[fromid].inhand
            ) {
                newtransaction.success = true;
                newtransaction.summary = "Transaction carried Successfully";

                companies[products[productId].company].net_worth +=
                    (products[productId].selling_price) *
                    quantity;
                companies[products[productId].company].income +=
                    (products[productId].selling_price -
                        products[productId].cost_price) *
                    quantity;
                decentralizedBank.transactions.push(tid);
                companies[products[productId].company].inhand +=
                    (products[productId].selling_price) *
                    quantity;
                companies[products[productId].company].transactions.push(tid);
                users[fromid].net_worth -=
                    (products[productId].selling_price -
                        products[productId].cost_price) *
                    quantity;
                users[fromid].inhand -=
    ...
    function takeloan(
        ...
        if (users[fromid].isUser && lenders[lenderid].islender) {
            ...
            Transaction memory newtransaction =
                Transaction(tid, true, lenderid, fromid, amount, false, "");
            users[lenderid].transactions.push(tid);
            users[fromid].transactions.push(tid);
            decentralizedBank.transactions.push(tid);
            users[lenderid].inhand -= amount;
            users[fromid].inhand += amount;
            users[lenderid].net_worth +=
                ((amount * lenders[lenderid].ipm) / 100) *
                month;
            users[fromid].net_worth -=
                ((amount * lenders[lenderid].ipm) / 100) *
                month;
            ...
            lenders[lenderid].amount.push(
                amount + ((amount * lenders[lenderid].ipm) / 100) * month
            );
            transactions[tid] = newtransaction;
        }
    }
    ...
    function createProduct(
        ...
        if (
            companies[companyId].isCompany &&
            cost_price * quantity <= companies[companyId].inhand
        ) {
            products_count += 1;
            bytes32 productId =
                stringToBytes32(
                    string(
                        abi.encodePacked("product_", uint2str(products_count))
                    )
                );
    ...
    function recruitEmployee(bytes32 companyId, bytes32 userId) external {
        if (
            companies[companyId].isCompany && companies[companyId].vacancy > 0
        ) {
    ...
    function userToUserTransaction(
        bytes32 fromid,
        bytes32 toid,
        int256 amount
    ) external {
